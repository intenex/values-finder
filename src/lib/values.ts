// The catalog of 93 standard values.
// IDs are immutable — they are referenced by saved results in the database.
// Ported verbatim from the original app's client/src/lib/values.ts.
...
export const VALUES: ValueDef[] = [
  ...
  { id: 68, name: "SELF-ACCEPTANCE", description: "to accept myself as I am" },
  { id: 69, name: "SAFETY", description: "to be safe and secure" },
  ...
  { id: 84, name: "PRESENCE", description: "to be fully engaged in the current moment" },
  { id: 85, name: "EMOTIONAL INTELLIGENCE", description: "to understand and work skillfully with my emotions" },
  { id: 86, name: "CATALYST", description: "to initiate positive change in systems and communities" },
  { id: 87, name: "INTEGRITY", description: "to align my actions with my deepest values" },
  { id: 88, name: "TRANSMISSION", description: "to share my spiritual experience to benefit others" },
  { id: 89, name: "VULNERABILITY", description: "to be open about my true feelings and experiences" },
  { id: 90, name: "IMPACT", description: "to create lasting positive change in the world" },
  { id: 91, name: "EQUANIMITY", description: "to maintain balance amid life's ups and downs" },
  { id: 92, name: "COMMUNITY", description: "to be part of a supportive growth-oriented group" },
  { id: 93, name: "AUTHENTICITY", description: "to live in alignment with my true nature" },
];

export const VALUE_COUNT = VALUES.length;

const byId = new Map(VALUES.map((v) => [v.id, v]));

export function getValue(id: number): ValueDef {
  const v = byId.get(id);
  if (!v) throw new Error(`Unknown value id: ${id}`);
  return v;
}
